...
const handleGetAllReviews = async (req, res) => {
  try {
    const reviewsCollection = mongoose.connection.db.collection('reviews');
    const allReviews = await reviewsCollection.find({}).sort({ createdAt: -1 }).limit(50).toArray();
    return res.status(200).json({
      success: true,
      reviews: allReviews || []
    });
...
// (Duplicate GET /reviews/user/:userId handled by handleGetUserReviews)
  ...
  try {
    ...
    const orderDocument = {
      ...
      items: cartItems.map(item => ({
        ...
        isFreeItem: Boolean(item.isFreeItem || item.isFree),
        isBogo: Boolean(item.isBogo),
        bogoTag: item.bogoTag || (item.isFreeItem ? '1+1 FREE' : (item.isBogo ? '1+1 Offer Applied' : null)),
      ...
      couponDiscount: req.body.couponDiscount !== undefined ? Number(req.body.couponDiscount) : (couponCode ? Number(discountAmount || 0) : 0),
      tieredDiscount: req.body.tieredDiscount !== undefined ? Number(req.body.tieredDiscount) : 0,
      tieredDiscountLabel: req.body.tieredDiscountLabel || null,
      totalSavings: req.body.totalSavings !== undefined ? Number(req.body.totalSavings) : (Number(discountAmount || 0) + Number(req.body.tieredDiscount || 0)),
...
// POST /reviews - Save review into MongoDB 'reviews' collection
const handleSaveReview = async (req, res) => {
  try {
    ...
    const finalUserId = String(userId || user_id || req.params.userid || req.params.userId || '').trim();
    const finalOrderId = String(orderId || order_id || req.params.orderId || '').trim();
    ...
    const reviewDoc = {
      ...
      restaurantReview: String(restaurantReview || review || '').trim(),
      deliveryBoyRating: Number(deliveryBoyRating ?? 0),
      deliveryBoyReview: String(deliveryBoyReview || '').trim(),
      orderDetails: orderDetails || [],
      createdAt: new Date(),
      updatedAt: new Date()
    };

    // Save directly into MongoDB 'reviews' collection
    await reviewsCollection.updateOne(
      { $or: [{ orderId: finalOrderId }, { order_id: finalOrderId }] },
      { $set: reviewDoc },
      { upsert: true }
    );
    ...
    return res.status(200).json({
      success: true,
      message: 'Review saved successfully to MongoDB reviews collection',
      review: reviewDoc
    });
  } catch (err) {
    console.error('Error saving review to MongoDB reviews collection:', err);
...
app.post('/api/reviews', handleSaveReview);
